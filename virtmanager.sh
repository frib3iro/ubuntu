...
echo "${seta} ${ciano}Instalando o KVM e pacotes adicionais de gerenciamento de virtualização${fim}"
sleep 2
if sudo apt install qemu-kvm libvirt-daemon-system libvirt-clients bridge-utils virtinst virt-manager -y
then
...
echo "${seta} ${ciano}Daemon libvirt será iniciado automaticamente${fim}"
sleep 2
...
echo "${seta} ${ciano}Adicionando seu usuário aos grupos libvirt e kvm${fim}"
sleep 2
...
echo "${seta} ${verde}Operação concluída com sucesso!${fim}"
